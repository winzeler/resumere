...
class UniqueHelper extends ChainHelperBase {
  ...
  async setCollectionSchemaVersion(signer, collectionId, schemaVersion = 'Unique', label='set schema version', transactionLabel='api.tx.unique.setSchemaVersion') {
    const result = await this.signTransaction(
      signer,
    ...
    if (result.status !== this.transactionStatus.SUCCESS) {
      throw Error(`Unable to set schema version for ${label}`);
    }
    return this.util.findCollectionInEvents(result.result.events, collectionId, 'common', 'SchemaVersionSet', label);
  }

  async setOffchainSchema(signer, collectionId, schemaString, label='set off chain schema', transactionLabel='api.tx.unique.setOffchainSchema') {
    const result = await this.signTransaction(
      signer,
      this.api.tx.unique.setOffchainSchema(collectionId, schemaString),
      transactionLabel
    );
    if (result.status !== this.transactionStatus.SUCCESS) {
      throw Error(`Unable to set off chain schema version for ${label}`);
    }
    return this.util.findCollectionInEvents(result.result.events, collectionId, 'common', 'OffchainSchemaSet', label);
  }
  ...
  async getCollectionSchemaVersion(collectionId) {    
    return (await this.api.rpc.unique.collectionProperties(collectionId, ['_old_schemaVersion']));

  }


  async getCollectionOffChainSchema(collectionId) {
    return (await this.api.rpc.unique.collectionProperties(collectionId, ['_old_offchainSchema']));

  }

  async getCollectionOnChainSchema(collectionId) {

    return (await this.api.rpc.unique.collectionProperties(collectionId, ['_old_constOnChainSchema']));
  ...
  async mintNFTToken(signer, { collectionId, owner, properties }, label = 'new token', transactionLabel = 'api.tx.unique.createItem') {
    const creationResult = await this.signTransaction(
      ...
      this.api.tx.unique.createItem(collectionId, (owner.Substrate || owner.Ethereum) ? owner : {Substrate: owner}, {
        //NFT: { const_data: Array.from(properties), variable_data: [] }
